...
단일객체에도 원자성/격리성은 적용된다.
20kb의 Json문서를 DB에 Write 한다고 가정해보자
- 첫 10kb를 전송 후 연결이 끊기면 파싱불가능한 10kb의 Json 조각을 저장할 것인가?
- 기존의 값을 덮어쓰는 도중 전원이 나간다면 기존값과 새값은 붙어있게 될 것인가?
- 문서를 쓰고 잇을 때 다른 클라이언트에서 그 문서를 읽으면 부분적으로 갱신된 값을 읽게 될까?
너무 다양한 고민은 혼란스러우므로 단일 노드에서 원자성과 격리성을 제공하는 것을 목표로 한다.
이를 위해 장애 복구용 로그나 각 객체에 잠금을 사용해(한쓰레드접근)서 구현가능하다.
atomic increment(7-1d의 read/modify/write를 없앤다)나 compare-and-set 연산으로 원자적연산을 제공하기도한다.( 추후 )
결론은, 일반적 트랜잭션은 다중객체 트랜잭션에 해당한다.
...
- 읽기를 실행하는 트랜잭션이 시작한 시점에 읽기대상객체를 생성한 트랜잭션이 이미 커밋된 상태였다.
- 읽기 대상 객체가 삭제된 것으로 표시되지 않았다. 또는 삭제된것으로 표시되었지만 읽기를 실행한 트랙잭션이 시작한 시점에 삭제요청 트랜잭션이 커밋되지 않았다.

오래 실행되는 트랜잭션은 오랫동안 스냅숏을 사용해서 덮어써지거나 삭제된지 오래된 값을 계속 읽을수도 있다.
데이터베이스는 갱신할 때 값을 교체하지 않고 값이 바뀔때마다 새버전을 생성함으로써 작은 오버헤드만 유발하면서 일관된 스냅숏을제공할 수도 있다.

##### 색인과 스냅숏 격리
다중버전 데이터 베이스에서의 색인은어떻게 동작할까?
- v1.0
  - 색인이 객체의 모든 버전을 가리키게 하고 색인 질의가 현재 트랜잭션에서 볼수 없는 버전을 걸러낸다. Garbage 컬렉션이 동작할때 색인도 삭제된다.
  - 예
    - PostgreSQL
      - 동일 객체의 다른 버전이 같은 페이지에 저장 될 수 있다면 색인 갱신을 회피하는 최적화를 한다.
    - 카우치DB, 데이토믹, LMDB 
      - B트리를 사용하지만 추가전용, append-only/copy-onwrite를 사용한다. ( 쓸때 복사되는 )
      - 트리의 루트에 이르기까지 존재하는 부모페이지들은 복사되고 그것들의 자식 페이지의 새버전을 가리키도록 갱신된다. 쓰기에 영향을 받지 않는 페이지들은 복사될 필요가 없고 변함없는 상태로 남는다.
      - 추가 전용 B 트리를 사용하면 쓰기를 실행하는 모든 트랜잭션은 새로운 B 트리 루트를 생성하며 특정 루트는 그것이 생성된 시점에 해당하는 DB의 일관된 스냅숏이 된다.
      - 나중에 실행되는 쓰기는 새로운 트리루트만 생서할 수 있고 존재하는 B 트리를 변경할 수 없으므로 트랜잭션 ID를 기반으로 객체를 걸러낼 필요가 없다. 별도의 compaction과 garbage 컬랙션
